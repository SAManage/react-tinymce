...
class TinyMCE extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (editor && editor.serializer && !isEqual(editor.getContent({format: 'html'}), nextProps.content)) {
      ...
      if (editor.getBody().children.length === 1) {
        // if the editor content contains only one element the select command is fails because the select function tries to find the next brother of the element
        const emptySpan = '<span></span>'
        editor.setContent(`${nextProps.content}${emptySpan}`);
      }
